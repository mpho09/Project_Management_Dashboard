...
export default function DashboardPage() {
  ...
  const filteredProjects = PROGRESS.filter((project) =>
    project.name.toLowerCase().includes(searchTerm.toLowerCase())
  );

  const filteredDeadlines = DEADLINES.filter((deadline) =>
    deadline.name.toLowerCase().includes(searchTerm.toLowerCase())
  );
      ...
      <header className="sticky top-0 z-10 flex items-center justify-between border-b border-line bg-surface/80 px-8 py-4 backdrop-blur">
        <div>
          ...
          <p className="text-xs text-ink-faint">
            Overview of everything on the go
          </p>
        </div>
      ...
      <main className="px-8 py-6">
        ...
        <section className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
          {STATS.map((s) => (
            <div
              key={s.label}
              className="rounded-xl border border-line bg-surface p-4 shadow-card transition hover:shadow-raised"
            >
              <p className="text-xs font-medium uppercase tracking-wide text-ink-faint">
                {s.label}
              </p>

              <p className={`mt-2 text-3xl font-bold ${toneRing[s.tone]}`}>
                {s.value}
              </p>
            ...
            <ul className="space-y-2.5">
              {filteredDeadlines.map((d) => (
                ...
                >
                  ...
                  <span className={`rounded-md px-2 py-0.5 text-xs font-semibold ${d.tone}`}>
                    {d.meta}
                  </span>
